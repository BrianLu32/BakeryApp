...
namespace YenHoungBakery
{
    public partial class YenHuongCalc : Form
    {
        ...
        //checking variables
        private ArrayList checkStoreList = new ArrayList();
        private int checkPriceChange = 0;
        private int checkHighestInvoiceNumber = 0;
        ...
        private void Form1_Load(object sender, EventArgs e)
        {
            try
            {
                //Gets project Directory for config file and database connection
                string workingDirectory = Environment.CurrentDirectory;
                string ParentDirectory = Directory.GetParent(workingDirectory).Parent.FullName;
                string checkConnection = ParentDirectory + "\\connectionFile.txt";
                string databaseConnection = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + ParentDirectory + "\\YenHoungBakeryDB2003.mdb";
                var connectionPath = File.ReadLines(checkConnection).Last();
                if (!File.Exists(checkConnection)) //Checks if the config file exists
                {
                    File.Create(checkConnection);
                    TextWriter tw = new StreamWriter(checkConnection);
                    tw.WriteLine(connectionPath);
                    tw.Close();
                }
                else if (File.Exists(checkConnection))
                {
            ...
            try
             {
                ...
                //Clears entries for each cake amount
                BanhBiaPriceAmountTxt.Clear(); SauRiengPriceAmountTxt.Clear();
                TrungThuPriceAmountTxt.Clear(); ThapCamPriceAmountTxt.Clear();
                BanhDeoPriceAmountTxt.Clear(); BanhSenPriceAmountTxt.Clear();
                BanhDauPriceAmountTxt.Clear(); DeoThapCamPriceAmountTxt.Clear();
                BanhInPriceAmountTxt.Clear(); RedBeanPriceAmountTxt.Clear();
                MochiPriceAmountTxt.Clear(); DuaTrungPriceAmountTxt.Clear();
                MutDauXanhPriceAmountTxt.Clear(); GreenTeaMangoPriceAmountTxt.Clear();
                BanhToLonPriceAmountTxt.Clear(); BanhToNhoPriceAmountTxt.Clear();
                NewYearCakePriceAmountTxt.Clear();
        ...
        private void ExitUpdateButton_Click(object sender, EventArgs e)
        {
            ...
            UpdateLabel.Text = ""; SelectFileButton.Enabled = true;
        }
        ...
        private void SelectFileButton_Click(object sender, EventArgs e)
        {
            if(openFile.ShowDialog() == DialogResult.OK)
            {
                ...
                {
                    //Retrieves file
                    string databasePath = openFile.FileName;
                    string path = Path.GetDirectoryName(databasePath);
                    string textFile = path + "connectionFile.txt";
                    string connectionString = "Provider=Microsoft.Jet.OLEDB.4.0;Data Source=" + databasePath;
                    connection.ConnectionString = connectionString;
                    
                    //creates a text file to store the connection string
                    if (!File.Exists(textFile))
                    {
                        File.Create(databasePath);
                        TextWriter tw = new StreamWriter(textFile);
                        tw.WriteLine(connectionString);
                        tw.Close();
                    }
                    else if (File.Exists(textFile))
                    {
                        var checkSameConnection = File.ReadLines(textFile).Last();
                        if (connectionString != checkSameConnection)
                        {
